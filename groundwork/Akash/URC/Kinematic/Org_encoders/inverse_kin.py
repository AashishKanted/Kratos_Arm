...
def inverse_kinematics_callback(msg):
    ...
    if check_angle_ranges(theta1, theta2, min_theta1, max_theta1, min_theta2, max_theta2):
        # Publish joint angles in degrees
        inv_kin_ang_pub.publish(Point(np.degrees(theta1), np.degrees(theta2), 0))
    else:
        # Publish 0 if angles are not within ranges
        inv_kin_ang_pub.publish(Point(0, 0, 0))
...
def inverse_kinematics(x, y, l1, l2):
    ...
    theta2 = theta2 + np.pi       # Makes it easier to visualise
    
    return theta1, theta2

def check_angle_ranges(theta1, theta2, min_theta1, max_theta1, min_theta2, max_theta2):
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('inv_kin_point', Float32MultiArray, inverse_kinematics_callback)

    # Publish joint angles in degrees to inv_kin_ang topic
    inv_kin_ang_pub = rospy.Publisher('inv_kin_ang', Point, queue_size=10)
