...
void messageCallback(const std_msgs::Int16MultiArray& receivedMsg)
{
  ...
  if(var == 0){
    ...
    digitalWrite(PWM6, LOW);  //linear actuator 1 extends
    digitalWrite(DIR4, HIGH);
  }
  else if(var == 2){
    analogWrite(PWM4, vel);
    digitalWrite(PWM6, LOW);  //linear actuator 1 retracts
    digitalWrite(DIR4, LOW);
  }
  else if(var == 3){
    digitalWrite(PWM4, LOW);
    analogWrite(PWM6, vel);  //linear actuator 2 extends
    digitalWrite(DIR6, HIGH);
  }
  else if(var == 4){
    digitalWrite(PWM4, LOW);
    analogWrite(PWM6, vel);  //linear actuator 2 retracts
    digitalWrite(DIR6, LOW);
  }
